...
export default async function Example() {
  ...
  const { data: prices, error: pricesError } = await supabase.from("prices_view").select().limit(1)

  const stats = [
    { id: 1, name: 'Prices', value: prices?.[0]?.pricescount, href: '/prices' },
    { id: 2, name: 'Products', value: prices?.[0]?.productscount, href: '/products' },
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl lg:text-center">
          ...
          <p className="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            You can verify what are the recent prices of products in many different stores
          </p>
